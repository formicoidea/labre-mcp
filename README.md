...
| &nbsp;&nbsp;door `supabase` | Supabase session JWTs (`Authorization: Bearer`) verified against the project JWKS. Requires `SUPABASE_URL`. The only door that reaches `agent.reply`'s RLS pass-through. |
