...
> **Surface actuelle** : 6 outils MCP câblés — `estimateEvolution`, `generateValueChain` (recette `generate`), `evaluateMap` (recette `evaluate-map`), `runCommand` (invocation directe de n'importe quel methodId), `runRecipe` (recettes multi-étapes shipped/override/bundle) et `__ping__` ; 86 stratégies (25 réelles / 61 mocks). L'écart à la cible est dans [roadmap.md](architecture/roadmap.md).
