...
Les outils MCP de labre-mcp dependent de services externes (LLM, BigQuery, web search). Le framework `src/lib/degradation/` factorise la facon dont ces outils signalent qu'une dependance est tombee — sans interrompre le pipeline ni masquer la panne.
...
client MCP --POST /mcp tools/call--> daemon (src/core/transport/)
                       ...
                       collector.wrap(payload) -> Degradable<T>
                              |
                              v (le daemon merge degraded + degradationEvents
...
```typescript
// Au boot du daemon (via l'API registerHealthCheck de src/lib/degradation/)
...
- `src/frameworks/wardley/evolution/_legacy/write/strategies/capacity/cpc-degradation.test.mts` — bout-en-bout sur la strategie CPC

Lancer : `npx tsx --test src/lib/degradation/*.test.mts`.
