...
1. **Strategy package (vertical slice).** Each `(command, strategy)` becomes a self-contained folder `frameworks/<domain>/<tool>/<subdomain>/<command>/<name>/` holding: `strategy.mts` (logic), its prompt pair(s) `<name>.system.md` / `<name>.user.md`, `schema.mts` (the I/O the strategy **owns**), and `meta.ts` (the `StrategyMetadata` of ast-schema § 3.4.3 — status, description, version, cost/latency/confidence class).
   - **Schemas placed by scope (three tiers), not a binary owned/central split.** A schema rises to the *smallest ancestor that shares it*:
     - **strategy-owned** (one strategy's I/O) → the strategy package `schema.mts`;
     - **métier tool/subdomain** (shared by the strategies of a tool or subdomain — e.g. `PurposeContext`, shared by `purpose:generate` and `purpose:audit-purpose-quality`) → `frameworks/<domain>/<tool>/schema.mts` or `.../<subdomain>/schema.mts`;
     - **kernel/transverse** (used by the engine everywhere — `RequestContext`, the JSON-labre envelope, `CommandCall`/`CommandResult`, the recipe schema, the methodId grammar) → central `src/schemas/`.

     This avoids both duplication (a shared métier contract is not copied into every package) and false centralisation (`PurposeContext` is not global — it is a `purpose`-subdomain contract).
...
3. **Prompts + recipes are business knowledge, DB-first.** On disk they are the **shipped seed / default**: prompts live inside the strategy package (§1); **recipes are tool-scoped (ARCH-06), co-located at the tool level** `frameworks/<domain>/<tool>/recipes/<name>.recipe.json` (a recipe orchestrates several strategies of its tool, so it belongs to the tool, not to any single strategy package). The canonical production store trends to the DB (continuing the bundle model), which resolves overrides by **methodId / recipe ref**, not file path; stdio and local dev keep running from the seed with no DB.
   - **Load model — runtime refresh, NOT build-time bake.** The DB is read **at boot into an in-memory cache and refreshed at runtime** (short TTL, or push-invalidated) — the wave-2 bundle read-model generalised (per call = memory only, zero DB queries; one probe per TTL window). It is deliberately **not** baked into the build artifact: A/B experiments and admin edits must go live **without a redeploy**, and `resources/list_changed` (§4) fires on catalogue changes. Baking at build would defeat both goals it is meant to serve; "fewer calls" comes from the cache, not from freezing at build.
