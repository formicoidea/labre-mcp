...
```javascript
import { logInfo, logDebug, logError, logWarning, setVerbose, isVerbose } from './mcp-notifications.mjs';
...
Le module `language-detect.mjs` utilise :
...
Le fichier `progress-messages.mjs` contient 40+ messages localises avec interpolation de variables :
...
Le module `llm-error-handler.mjs` classe les erreurs LLM en types specifiques :
...
```javascript
import { classifyAndLogLLMError, withLLMErrorLogging } from './llm-error-handler.mjs';
