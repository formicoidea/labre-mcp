...
export function buildApp(options: {
  ...
  // Published JSON Schemas of the data contract (schema/, regenerated by
  // `npm run schemas`). Public like /health — schemas are the spec, not data.
  ...
  app.get("/schemas/:file", async (c) => {
    ...
    try {
      const body = await readFile(new URL(file, schemaDir), "utf8");
      return c.body(body, 200, { "Content-Type": "application/schema+json" });
    } catch {
      return c.json({ error: "not found" }, 404);
    }
  });
