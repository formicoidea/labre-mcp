// Strategy registry for read:component:* strategies.
//
// Auto-discovers all *-strategy.{mjs,mts} files in this directory that export
// a class extending BaseComponentReadStrategy. Pattern mirrors
// src/work-on-evolution/write/strategies/capacity/registry.mts.

import { readdir } from 'node:fs/promises';
import { fileURLToPath, pathToFileURL } from 'node:url';
import { dirname, join } from 'node:path';
import { BaseComponentReadStrategy } from './base-strategy.mjs';

const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);

export type StrategyCtor = (new (...args: any[]) => BaseComponentReadStrategy) & {
  method: string;
  ...
  for (const file of strategyFiles) {
    ...
    for (const exportName of Object.keys(mod)) {
      ...
      if (
        ...
        (candidate as Function).prototype instanceof BaseComponentReadStrategy &&
        candidate !== BaseComponentReadStrategy
      ) {
