// Strategy registry for write:component:* strategies.
//
// Auto-discovers all *-strategy.{mjs,mts} files in this directory or in
// any one-level sub-folder (e.g. strategies/<method>/<method>-strategy.mts)
// that export a class extending BaseComponentWriteStrategy.

import { readdir } from 'node:fs/promises';
import { fileURLToPath, pathToFileURL } from 'node:url';
import { dirname, join } from 'node:path';
import { BaseComponentWriteStrategy } from './base-strategy.mjs';

const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);

export type StrategyCtor = (new (...args: any[]) => BaseComponentWriteStrategy) & {
  method: string;
  ...
  for (const fullPath of strategyPaths) {
    ...
    for (const exportName of Object.keys(mod)) {
      ...
      if (
        ...
        (candidate as Function).prototype instanceof BaseComponentWriteStrategy &&
        candidate !== BaseComponentWriteStrategy
      ) {
