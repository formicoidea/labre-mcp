...
//   (f) mapWidth scales with the densest ±DENSITY_WINDOW_HALF window
...
describe('adjustX — clamp & preservation (a, b)', () => {
  ...
  it('keeps single-consumer components within ±BAND_HALF of their xHint', () => {
    const out = adjustX(seed(
      [
...
describe('adjustX — mapWidth (f)', () => {
  function chainWithKHints(K: number): PositionedValueChain {
    // K components inside the same ±DENSITY_WINDOW_HALF window (here 0.45..0.55).
  ...
  it('keeps mapWidth at BASE when K_max ≤ DENSITY_LIMIT_K', () => {
    // Build a chain whose densest ±0.05 window holds DENSITY_LIMIT_K
