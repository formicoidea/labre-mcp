// End-to-end test for NarrativeChainStrategy with a mocked llmCall.
//
...
describe('NarrativeChainStrategy', () => {
  ...
  it('registers under write:chain:narrative', async () => {
    ...
    assert.ok(
      methods.includes('write:chain:narrative'),
      `expected write:chain:narrative in registry, got: ${methods.join(', ')}`,
    );
  });

  it('is retrievable via getStrategy', async () => {
    const Cls = await getStrategy('write:chain:narrative');
    assert.equal(Cls.method, 'write:chain:narrative');
    assert.equal(Cls.name, 'NarrativeChainStrategy');
  });

  it('requires llmCall in the constructor', () => {
    // any: bypass typed constructor to exercise runtime check
    assert.throws(() => new (NarrativeChainStrategy as any)({}),
      /requires an llmCall function/);
  });

  it('rejects an empty nlCommand', async () => {
    const strat = new NarrativeChainStrategy({ llmCall: makeMockLlm() });
