...
import { identifyCapability, parseCapabilityResponse } from './identify-capability.mjs';
...
if (process.argv[1] && import.meta.url === `file:///${process.argv[1].replace(/\\/g, '/')}`) {
  console.log('=== pipeline-capability-inference.mjs self-test ===\n');

  // ── Test 1: capabilityToLabel transformations ───────────────────────
  console.log('--- Test 1: capabilityToLabel ---');
  const labelTests = [
    { cap: 'Orchestrate containers', nature: 'activity', expected: 'Containers Orchestration' },
    { cap: 'Manage customer relationships', nature: 'activity', expected: 'Customer Relationships Management' },
    { cap: 'how to manage IT services', nature: 'practice', expected: 'IT Services Management' },
    { cap: 'how to manage a project', nature: 'practice', expected: 'A Project Management' },
    { cap: 'technical expertise in welding', nature: 'knowledge', expected: 'Welding Expertise' },
    { cap: 'interpersonal skills for coaching', nature: 'knowledge', expected: 'Coaching Skills' },
    { cap: 'ambient temperature', nature: 'data', expected: 'Ambient Temperature' },
    { cap: 'Deploy microservices', nature: 'activity', expected: 'Microservices Deployment' },
    { cap: '', nature: 'activity', expected: 'Capability' },
  ];

  for (const lt of labelTests) {
    const result = capabilityToLabel(lt.cap, lt.nature);
    const ok = result === lt.expected;
    console.log(`  ${ok ? '✓' : '✗'} "${lt.cap}" (${lt.nature}) → "${result}"${ok ? '' : ` (expected: "${lt.expected}")`}`);
  }
  ...
  const titleTests = [
    ...
    { input: 'IT service management', expected: 'IT Service Management' },
    { input: 'CRM systems', expected: 'CRM Systems' },
  ];

  for (const tt of titleTests) {
    const result = titleCase(tt.input);
    const ok = result === tt.expected;
    console.log(`  ${ok ? '✓' : '✗'} "${tt.input}" → "${result}"${ok ? '' : ` (expected: "${tt.expected}")`}`);
  }
  ...
  const result = await inferCapabilityFromSolution('Kubernetes', {
    description: 'Container orchestration platform by Google',
    llmCall: mockLLM,
  });

  console.log(`  Solution: ${result.solutionName}`);
  console.log(`  Capability: ${result.capability}`);
  console.log(`  Label: ${result.capabilityLabel}`);
  console.log(`  Nature: ${result.nature}`);
  console.log(`  Type: ${result.wardleyType}`);
  console.log(`  Confidence: ${result.confidence}`);
  console.log(`  Inferred: ${result.inferred}`);

  const ok = result.solutionName === 'Kubernetes'
    && result.capability === 'Orchestrate containers'
    && result.capabilityLabel === 'Containers Orchestration'
    && result.nature === 'activity'
    && result.confidence === 0.92
    && result.inferred === true;
  console.log(`  ${ok ? '✓' : '✗'} Full inference pipeline ${ok ? 'passed' : 'FAILED'}`);

  // ── Test 5: Methodology inference ──────────────────────────────────
  console.log('\n--- Test 5: Methodology inference ---');
  const mockMethodology = async () => [
    'type=component',
    'nature=practice',
  ...
  const scrumResult = await inferCapabilityFromSolution('Scrum', {
    description: 'Agile project management framework',
    llmCall: mockMethodology,
  });

  console.log(`  Solution: ${scrumResult.solutionName}`);
  console.log(`  Capability: ${scrumResult.capability}`);
  console.log(`  Label: ${scrumResult.capabilityLabel}`);
  console.log(`  Nature: ${scrumResult.nature}`);
  console.log(`  ${scrumResult.nature === 'practice' ? '✓' : '✗'} Correctly identified as practice`);
