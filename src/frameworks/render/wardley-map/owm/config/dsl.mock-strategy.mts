// Mock strategy for `render:wardley-map:owm:config:dsl` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'render:wardley-map:owm:config:dsl';

export interface RenderWardleyMapOwmConfigDslInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface RenderWardleyMapOwmConfigDslResult {
  mock: true;
  methodId: string;
...
export class MockRenderWardleyMapOwmConfigDslStrategy extends CoreBaseStrategy<RenderWardleyMapOwmConfigDslInput, RenderWardleyMapOwmConfigDslResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: RenderWardleyMapOwmConfigDslInput,
    _context: RequestContext,
  ): Promise<StrategyResult<RenderWardleyMapOwmConfigDslResult>> {
