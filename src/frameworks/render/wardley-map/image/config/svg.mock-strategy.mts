// Mock strategy for `render:wardley-map:image:config:svg` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'render:wardley-map:image:config:svg';

export interface RenderWardleyMapImageConfigSvgInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface RenderWardleyMapImageConfigSvgResult {
  mock: true;
  methodId: string;
...
export class MockRenderWardleyMapImageConfigSvgStrategy extends CoreBaseStrategy<RenderWardleyMapImageConfigSvgInput, RenderWardleyMapImageConfigSvgResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: RenderWardleyMapImageConfigSvgInput,
    _context: RequestContext,
  ): Promise<StrategyResult<RenderWardleyMapImageConfigSvgResult>> {
