// Mock strategy for `render:wardley-map:image:config:png` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'render:wardley-map:image:config:png';

export interface RenderWardleyMapImageConfigPngInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface RenderWardleyMapImageConfigPngResult {
  mock: true;
  methodId: string;
...
export class MockRenderWardleyMapImageConfigPngStrategy extends CoreBaseStrategy<RenderWardleyMapImageConfigPngInput, RenderWardleyMapImageConfigPngResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: RenderWardleyMapImageConfigPngInput,
    _context: RequestContext,
  ): Promise<StrategyResult<RenderWardleyMapImageConfigPngResult>> {
