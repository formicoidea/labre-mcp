// Mock strategy for `wardley:iteration:strategy-cycle:explain:default` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:iteration:strategy-cycle:explain:default';

export interface WardleyIterationStrategyCycleExplainDefaultInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyIterationStrategyCycleExplainDefaultResult {
  mock: true;
  methodId: string;
...
export class MockWardleyIterationStrategyCycleExplainDefaultStrategy extends CoreBaseStrategy<WardleyIterationStrategyCycleExplainDefaultInput, WardleyIterationStrategyCycleExplainDefaultResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyIterationStrategyCycleExplainDefaultInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyIterationStrategyCycleExplainDefaultResult>> {
