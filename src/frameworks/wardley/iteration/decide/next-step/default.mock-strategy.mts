// Mock strategy for `wardley:iteration:decide:next-step:default` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:iteration:decide:next-step:default';

export interface WardleyIterationDecideNextStepDefaultInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyIterationDecideNextStepDefaultResult {
  mock: true;
  methodId: string;
...
export class MockWardleyIterationDecideNextStepDefaultStrategy extends CoreBaseStrategy<WardleyIterationDecideNextStepDefaultInput, WardleyIterationDecideNextStepDefaultResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyIterationDecideNextStepDefaultInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyIterationDecideNextStepDefaultResult>> {
