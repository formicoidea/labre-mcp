// Mock strategy for `wardley:iteration:purpose:audit-purpose-quality:default` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
export interface WardleyIterationPurposeAuditPurposeQualityDefaultInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyIterationPurposeAuditPurposeQualityDefaultResult {
  mock: true;
  methodId: string;
...
export class MockWardleyIterationPurposeAuditPurposeQualityDefaultStrategy extends CoreBaseStrategy<WardleyIterationPurposeAuditPurposeQualityDefaultInput, WardleyIterationPurposeAuditPurposeQualityDefaultResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyIterationPurposeAuditPurposeQualityDefaultInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyIterationPurposeAuditPurposeQualityDefaultResult>> {
