// Mock strategy for `wardley:iteration:purpose:generate:default` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
export interface WardleyIterationPurposeGenerateDefaultInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyIterationPurposeGenerateDefaultResult {
  mock: true;
  methodId: string;
...
export class MockWardleyIterationPurposeGenerateDefaultStrategy extends CoreBaseStrategy<WardleyIterationPurposeGenerateDefaultInput, WardleyIterationPurposeGenerateDefaultResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyIterationPurposeGenerateDefaultInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyIterationPurposeGenerateDefaultResult>> {
