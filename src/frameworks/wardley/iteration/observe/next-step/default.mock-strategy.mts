// Mock strategy for `wardley:iteration:observe:next-step:default` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:iteration:observe:next-step:default';

export interface WardleyIterationObserveNextStepDefaultInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyIterationObserveNextStepDefaultResult {
  mock: true;
  methodId: string;
...
export class MockWardleyIterationObserveNextStepDefaultStrategy extends CoreBaseStrategy<WardleyIterationObserveNextStepDefaultInput, WardleyIterationObserveNextStepDefaultResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyIterationObserveNextStepDefaultInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyIterationObserveNextStepDefaultResult>> {
