// Mock strategy for `wardley:iteration:why-of-purpose:guide:default` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:iteration:why-of-purpose:guide:default';

export interface WardleyIterationWhyOfPurposeGuideDefaultInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyIterationWhyOfPurposeGuideDefaultResult {
  mock: true;
  methodId: string;
...
export class MockWardleyIterationWhyOfPurposeGuideDefaultStrategy extends CoreBaseStrategy<WardleyIterationWhyOfPurposeGuideDefaultInput, WardleyIterationWhyOfPurposeGuideDefaultResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyIterationWhyOfPurposeGuideDefaultInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyIterationWhyOfPurposeGuideDefaultResult>> {
