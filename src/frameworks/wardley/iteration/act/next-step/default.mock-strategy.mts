// Mock strategy for `wardley:iteration:act:next-step:default` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:iteration:act:next-step:default';

export interface WardleyIterationActNextStepDefaultInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyIterationActNextStepDefaultResult {
  mock: true;
  methodId: string;
...
export class MockWardleyIterationActNextStepDefaultStrategy extends CoreBaseStrategy<WardleyIterationActNextStepDefaultInput, WardleyIterationActNextStepDefaultResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyIterationActNextStepDefaultInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyIterationActNextStepDefaultResult>> {
