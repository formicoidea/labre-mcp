// Strategy registry for write:anchor:* strategies.
//
// Auto-discovers all *-strategy.{mjs,mts} files in this directory that export
// a class extending BaseAnchorWriteStrategy. Pattern mirrors
// src/work-on-evolution/write/strategies/capacity/registry.mts.
//
// Adding a new write-anchor strategy requires ONLY creating a new file here.
// No modifications to this registry or any other existing code are needed.

import { readdir } from 'node:fs/promises';
import { fileURLToPath, pathToFileURL } from 'node:url';
import { dirname, join } from 'node:path';
import { BaseAnchorWriteStrategy } from './base-strategy.mjs';

const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);

export type StrategyCtor = (new (...args: any[]) => BaseAnchorWriteStrategy) & {
  method: string;
  ...
  for (const file of strategyFiles) {
    ...
    for (const exportName of Object.keys(mod)) {
      ...
      if (
        ...
        (candidate as Function).prototype instanceof BaseAnchorWriteStrategy &&
        candidate !== BaseAnchorWriteStrategy
      ) {
