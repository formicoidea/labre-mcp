// Mock strategy for `wardley:doctrine:wiki:doctrinal-analysis:default` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:doctrine:wiki:doctrinal-analysis:default';

export interface WardleyDoctrineWikiDoctrinalAnalysisDefaultInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyDoctrineWikiDoctrinalAnalysisDefaultResult {
  mock: true;
  methodId: string;
...
export class MockWardleyDoctrineWikiDoctrinalAnalysisDefaultStrategy extends CoreBaseStrategy<WardleyDoctrineWikiDoctrinalAnalysisDefaultInput, WardleyDoctrineWikiDoctrinalAnalysisDefaultResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyDoctrineWikiDoctrinalAnalysisDefaultInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyDoctrineWikiDoctrinalAnalysisDefaultResult>> {
