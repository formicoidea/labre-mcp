// Mock strategy for `wardley:doctrine:wiki:list:kanban-view` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:doctrine:wiki:list:kanban-view';

export interface WardleyDoctrineWikiListKanbanViewInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyDoctrineWikiListKanbanViewResult {
  mock: true;
  methodId: string;
...
export class MockWardleyDoctrineWikiListKanbanViewStrategy extends CoreBaseStrategy<WardleyDoctrineWikiListKanbanViewInput, WardleyDoctrineWikiListKanbanViewResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyDoctrineWikiListKanbanViewInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyDoctrineWikiListKanbanViewResult>> {
