// Mock strategy for `wardley:doctrine:wiki:list:phase-view` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:doctrine:wiki:list:phase-view';

export interface WardleyDoctrineWikiListPhaseViewInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyDoctrineWikiListPhaseViewResult {
  mock: true;
  methodId: string;
...
export class MockWardleyDoctrineWikiListPhaseViewStrategy extends CoreBaseStrategy<WardleyDoctrineWikiListPhaseViewInput, WardleyDoctrineWikiListPhaseViewResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyDoctrineWikiListPhaseViewInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyDoctrineWikiListPhaseViewResult>> {
