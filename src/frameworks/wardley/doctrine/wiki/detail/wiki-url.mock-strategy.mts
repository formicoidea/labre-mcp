// Mock strategy for `wardley:doctrine:wiki:detail:wiki-url` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:doctrine:wiki:detail:wiki-url';

export interface WardleyDoctrineWikiDetailWikiUrlInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyDoctrineWikiDetailWikiUrlResult {
  mock: true;
  methodId: string;
...
export class MockWardleyDoctrineWikiDetailWikiUrlStrategy extends CoreBaseStrategy<WardleyDoctrineWikiDetailWikiUrlInput, WardleyDoctrineWikiDetailWikiUrlResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyDoctrineWikiDetailWikiUrlInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyDoctrineWikiDetailWikiUrlResult>> {
