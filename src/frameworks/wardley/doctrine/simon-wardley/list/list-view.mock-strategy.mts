// Mock strategy for `wardley:doctrine:simon-wardley:list:list-view` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:doctrine:simon-wardley:list:list-view';

export interface WardleyDoctrineSimonWardleyListListViewInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyDoctrineSimonWardleyListListViewResult {
  mock: true;
  methodId: string;
...
export class MockWardleyDoctrineSimonWardleyListListViewStrategy extends CoreBaseStrategy<WardleyDoctrineSimonWardleyListListViewInput, WardleyDoctrineSimonWardleyListListViewResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyDoctrineSimonWardleyListListViewInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyDoctrineSimonWardleyListListViewResult>> {
