// Mock strategy for `wardley:doctrine:simon-wardley:list:kanban-view` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:doctrine:simon-wardley:list:kanban-view';

export interface WardleyDoctrineSimonWardleyListKanbanViewInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyDoctrineSimonWardleyListKanbanViewResult {
  mock: true;
  methodId: string;
...
export class MockWardleyDoctrineSimonWardleyListKanbanViewStrategy extends CoreBaseStrategy<WardleyDoctrineSimonWardleyListKanbanViewInput, WardleyDoctrineSimonWardleyListKanbanViewResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyDoctrineSimonWardleyListKanbanViewInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyDoctrineSimonWardleyListKanbanViewResult>> {
