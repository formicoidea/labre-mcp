...
//
// Wraps the legacy `LogprobDistributionStrategy` in the core BaseStrategy
// contract (StrategyResult shape, 5-segment methodId).
...
export class LogprobDistributionStrategyCore extends CoreBaseStrategy<ComponentInput, EvolutionResult> {
  ...
  async evaluate(
    ...
    const result = await legacy.evaluate(component) as EvolutionResult & {
      trace?: Array<{ distribution?: Record<string, number> }>;
    };
    const wrapped: EvolutionResult = { ...result, method: NEW_METHOD_ID_LOGPROB };

    const capturedAt = new Date().toISOString();
    const distribution = result.trace?.[0]?.distribution;
    const signals = [
      ...
      ...(component.context
        ...
        : []),
      ...(distribution
        ? [{ name: 'distribution', value: distribution, source: 'llm-internal' as const, capturedAt }]
        : []),
    ];
    // Peak phase + concentration insight: which phase carries the most mass?
    let insights: StrategyResult<EvolutionResult>['insights'] = [];
    if (distribution) {
      const entries = Object.entries(distribution);
      const peak = entries.reduce((a, b) => (b[1] > a[1] ? b : a));
      insights = [{
        text: `Peak phase: ${peak[0]} with probability ${peak[1].toFixed(3)} (confidence ${result.confidence.toFixed(3)} from distribution entropy)`,
        by: NEW_METHOD_ID_LOGPROB,
