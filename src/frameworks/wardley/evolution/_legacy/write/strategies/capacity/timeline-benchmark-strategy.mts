...
export class TimelineBenchmarkStrategy extends BaseStrategy {
  ...
  async evaluate(component: ComponentInput): Promise<EvolutionResult> {
    ...
    for (let i = 0; i < MAX_HISTORY_ITERATIONS; i++) {
      ...
      let milestone;
      try {
        const response = await this._llmCall(iterationBuilt.user, undefined, { systemPrompt: iterationBuilt.system });
      ...
      // LLMDirectStrategy returns a StrategyResult — pass a fake context and unwrap .result.
      let evoResult;
      try {
        const wrapped = await llmDirect.evaluate(
          {
            // any: timeline-benchmark passes a 'kind' field that ComponentInput doesn't declare — open shape
            kind: 'capability',
            name: milestone.name,
            capability: capability.capability,
          ...
          // any: synthetic context — timeline-benchmark is itself legacy, no real RequestContext available
          {} as any,
        );
        evoResult = wrapped.result;
      } catch (err) {
        if (history.length > 0) break;
        throw err;
      }
    ...
    if (history.length > 0 && history[history.length - 1].date < CURRENT_YEAR) {
      try {
        const wrapped = await llmDirect.evaluate(
          ...
          // any: synthetic context
          {} as any,
        );
        const presentResult = wrapped.result;
        history.push({
...
export class TimelineBenchmarkStrategyCore extends CoreBaseStrategy<ComponentInput, EvolutionResult> {
  ...
  async evaluate(
    ...
    const wrapped: EvolutionResult = { ...result, method: NEW_METHOD_ID_TB };

    const capturedAt = new Date().toISOString();
    const signals = [
      { name: 'component', value: component.name ?? '', source: 'user-input' as const, capturedAt },
      ...(component.capability
        ? [{ name: 'capability', value: component.capability, source: 'user-input' as const, capturedAt }]
        : []),
      ...(component.date
        ? [{ name: 'date', value: String(component.date), source: 'user-input' as const, capturedAt }]
        : []),
    ];
    // Each milestone in the timeline becomes an insight describing the trajectory.
    const history = (result.trace ?? []) as Milestone[];
    const insights = history.map((m) => ({
      text: `Milestone "${m.name}" at ${m.date}: evolution=${m.evolution}, confidence=${m.confidence}${m._fallback ? ' (fallback present-day)' : ''}`,
      by: NEW_METHOD_ID_TB,
