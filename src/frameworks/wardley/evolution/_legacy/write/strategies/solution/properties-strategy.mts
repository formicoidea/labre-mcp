...
export class PropertiesStrategyCore extends CoreBaseStrategy<SolutionInput, SolutionEvolutionResult> {
  ...
  async evaluate(
    ...
    const result = await legacy.evaluate(component) as SolutionEvolutionResult & {
      properties?: Array<{ property: string; phase?: string | number; reason?: string; confidence?: number }>;
    };
    const wrapped: SolutionEvolutionResult = { ...result, method: NEW_METHOD_ID_PROPERTIES };

    const capturedAt = new Date().toISOString();
    const signals = [
      { name: 'solution', value: component.name ?? '', source: 'user-input' as const, capturedAt },
      ...(component.context
    ...
    const insights = (result.properties ?? [])
      .filter((p) => p.reason)
      .map((p) => ({
        text: `${p.property}${p.phase != null ? ` → ${p.phase}` : ''}: ${p.reason}`,
        by: NEW_METHOD_ID_PROPERTIES,
        type: 'other' as const,
      }));
    return {
      signals,
