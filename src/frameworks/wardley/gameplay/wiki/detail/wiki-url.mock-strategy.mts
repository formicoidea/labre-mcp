// Mock strategy for `wardley:gameplay:wiki:detail:wiki-url` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:gameplay:wiki:detail:wiki-url';

export interface WardleyGameplayWikiDetailWikiUrlInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyGameplayWikiDetailWikiUrlResult {
  mock: true;
  methodId: string;
...
export class MockWardleyGameplayWikiDetailWikiUrlStrategy extends CoreBaseStrategy<WardleyGameplayWikiDetailWikiUrlInput, WardleyGameplayWikiDetailWikiUrlResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyGameplayWikiDetailWikiUrlInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyGameplayWikiDetailWikiUrlResult>> {
