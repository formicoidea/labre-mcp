// Mock strategy for `wardley:gameplay:simon-wardley:list:list-view` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:gameplay:simon-wardley:list:list-view';

export interface WardleyGameplaySimonWardleyListListViewInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyGameplaySimonWardleyListListViewResult {
  mock: true;
  methodId: string;
...
export class MockWardleyGameplaySimonWardleyListListViewStrategy extends CoreBaseStrategy<WardleyGameplaySimonWardleyListListViewInput, WardleyGameplaySimonWardleyListListViewResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyGameplaySimonWardleyListListViewInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyGameplaySimonWardleyListListViewResult>> {
