// Mock strategy for `wardley:climate:wiki:list:kanban-view` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:climate:wiki:list:kanban-view';

export interface WardleyClimateWikiListKanbanViewInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyClimateWikiListKanbanViewResult {
  mock: true;
  methodId: string;
...
export class MockWardleyClimateWikiListKanbanViewStrategy extends CoreBaseStrategy<WardleyClimateWikiListKanbanViewInput, WardleyClimateWikiListKanbanViewResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyClimateWikiListKanbanViewInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyClimateWikiListKanbanViewResult>> {
