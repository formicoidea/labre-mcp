// Mock strategy for `wardley:climate:wiki:list:list-view` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:climate:wiki:list:list-view';

export interface WardleyClimateWikiListListViewInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyClimateWikiListListViewResult {
  mock: true;
  methodId: string;
...
export class MockWardleyClimateWikiListListViewStrategy extends CoreBaseStrategy<WardleyClimateWikiListListViewInput, WardleyClimateWikiListListViewResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyClimateWikiListListViewInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyClimateWikiListListViewResult>> {
