// Mock strategy for `wardley:climate:wiki:detail:wiki-url` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:climate:wiki:detail:wiki-url';

export interface WardleyClimateWikiDetailWikiUrlInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyClimateWikiDetailWikiUrlResult {
  mock: true;
  methodId: string;
...
export class MockWardleyClimateWikiDetailWikiUrlStrategy extends CoreBaseStrategy<WardleyClimateWikiDetailWikiUrlInput, WardleyClimateWikiDetailWikiUrlResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyClimateWikiDetailWikiUrlInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyClimateWikiDetailWikiUrlResult>> {
