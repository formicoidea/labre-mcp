// Mock strategy for `wardley:climate:simon-wardley:list:list-view` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:climate:simon-wardley:list:list-view';

export interface WardleyClimateSimonWardleyListListViewInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyClimateSimonWardleyListListViewResult {
  mock: true;
  methodId: string;
...
export class MockWardleyClimateSimonWardleyListListViewStrategy extends CoreBaseStrategy<WardleyClimateSimonWardleyListListViewInput, WardleyClimateSimonWardleyListListViewResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyClimateSimonWardleyListListViewInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyClimateSimonWardleyListListViewResult>> {
