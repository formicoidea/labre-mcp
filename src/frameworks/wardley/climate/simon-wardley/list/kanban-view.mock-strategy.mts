// Mock strategy for `wardley:climate:simon-wardley:list:kanban-view` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:climate:simon-wardley:list:kanban-view';

export interface WardleyClimateSimonWardleyListKanbanViewInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyClimateSimonWardleyListKanbanViewResult {
  mock: true;
  methodId: string;
...
export class MockWardleyClimateSimonWardleyListKanbanViewStrategy extends CoreBaseStrategy<WardleyClimateSimonWardleyListKanbanViewInput, WardleyClimateSimonWardleyListKanbanViewResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyClimateSimonWardleyListKanbanViewInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyClimateSimonWardleyListKanbanViewResult>> {
