// Mock strategy for `wardley:map:output:update:default` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:map:output:update:default';

export interface WardleyMapOutputUpdateDefaultInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyMapOutputUpdateDefaultResult {
  mock: true;
  methodId: string;
...
export class MockWardleyMapOutputUpdateDefaultStrategy extends CoreBaseStrategy<WardleyMapOutputUpdateDefaultInput, WardleyMapOutputUpdateDefaultResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyMapOutputUpdateDefaultInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyMapOutputUpdateDefaultResult>> {
