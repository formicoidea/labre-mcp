// Mock strategy for `wardley:map:output:read:where-to-invest` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:map:output:read:where-to-invest';

export interface WardleyMapOutputReadWhereToInvestInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyMapOutputReadWhereToInvestResult {
  mock: true;
  methodId: string;
...
export class MockWardleyMapOutputReadWhereToInvestStrategy extends CoreBaseStrategy<WardleyMapOutputReadWhereToInvestInput, WardleyMapOutputReadWhereToInvestResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyMapOutputReadWhereToInvestInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyMapOutputReadWhereToInvestResult>> {
