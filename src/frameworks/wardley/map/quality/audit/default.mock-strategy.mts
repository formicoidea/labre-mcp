// Mock strategy for `wardley:map:quality:audit:default` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:map:quality:audit:default';

export interface WardleyMapQualityAuditDefaultInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyMapQualityAuditDefaultResult {
  mock: true;
  methodId: string;
...
export class MockWardleyMapQualityAuditDefaultStrategy extends CoreBaseStrategy<WardleyMapQualityAuditDefaultInput, WardleyMapQualityAuditDefaultResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyMapQualityAuditDefaultInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyMapQualityAuditDefaultResult>> {
