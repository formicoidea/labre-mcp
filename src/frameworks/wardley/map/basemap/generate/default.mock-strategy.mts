// Mock strategy for `wardley:map:basemap:generate:default` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
export interface WardleyMapBasemapGenerateDefaultInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyMapBasemapGenerateDefaultResult {
  mock: true;
  methodId: string;
...
export class MockWardleyMapBasemapGenerateDefaultStrategy extends CoreBaseStrategy<WardleyMapBasemapGenerateDefaultInput, WardleyMapBasemapGenerateDefaultResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyMapBasemapGenerateDefaultInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyMapBasemapGenerateDefaultResult>> {
