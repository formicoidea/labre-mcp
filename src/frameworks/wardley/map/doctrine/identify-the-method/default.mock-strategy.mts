// Mock strategy for `wardley:map:doctrine:identify-the-method:default` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:map:doctrine:identify-the-method:default';

export interface WardleyMapDoctrineIdentifyTheMethodDefaultInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyMapDoctrineIdentifyTheMethodDefaultResult {
  mock: true;
  methodId: string;
...
export class MockWardleyMapDoctrineIdentifyTheMethodDefaultStrategy extends CoreBaseStrategy<WardleyMapDoctrineIdentifyTheMethodDefaultInput, WardleyMapDoctrineIdentifyTheMethodDefaultResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyMapDoctrineIdentifyTheMethodDefaultInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyMapDoctrineIdentifyTheMethodDefaultResult>> {
