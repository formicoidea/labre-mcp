// Mock strategy for `wardley:map:config:x-axis:custom` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:map:config:x-axis:custom';

export interface WardleyMapConfigXAxisCustomInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyMapConfigXAxisCustomResult {
  mock: true;
  methodId: string;
...
export class MockWardleyMapConfigXAxisCustomStrategy extends CoreBaseStrategy<WardleyMapConfigXAxisCustomInput, WardleyMapConfigXAxisCustomResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyMapConfigXAxisCustomInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyMapConfigXAxisCustomResult>> {
