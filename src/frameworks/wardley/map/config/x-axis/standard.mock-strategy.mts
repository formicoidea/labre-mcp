// Mock strategy for `wardley:map:config:x-axis:standard` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:map:config:x-axis:standard';

export interface WardleyMapConfigXAxisStandardInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyMapConfigXAxisStandardResult {
  mock: true;
  methodId: string;
...
export class MockWardleyMapConfigXAxisStandardStrategy extends CoreBaseStrategy<WardleyMapConfigXAxisStandardInput, WardleyMapConfigXAxisStandardResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyMapConfigXAxisStandardInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyMapConfigXAxisStandardResult>> {
