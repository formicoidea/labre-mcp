// Mock strategy for `wardley:map:config:y-axis:standard` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:map:config:y-axis:standard';

export interface WardleyMapConfigYAxisStandardInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyMapConfigYAxisStandardResult {
  mock: true;
  methodId: string;
...
export class MockWardleyMapConfigYAxisStandardStrategy extends CoreBaseStrategy<WardleyMapConfigYAxisStandardInput, WardleyMapConfigYAxisStandardResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyMapConfigYAxisStandardInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyMapConfigYAxisStandardResult>> {
