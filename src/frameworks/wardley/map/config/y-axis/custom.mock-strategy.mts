// Mock strategy for `wardley:map:config:y-axis:custom` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:map:config:y-axis:custom';

export interface WardleyMapConfigYAxisCustomInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyMapConfigYAxisCustomResult {
  mock: true;
  methodId: string;
...
export class MockWardleyMapConfigYAxisCustomStrategy extends CoreBaseStrategy<WardleyMapConfigYAxisCustomInput, WardleyMapConfigYAxisCustomResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyMapConfigYAxisCustomInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyMapConfigYAxisCustomResult>> {
