// Mock strategy for `wardley:map:node:identify-point-of-change:default` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:map:node:identify-point-of-change:default';

export interface WardleyMapNodeIdentifyPointOfChangeDefaultInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyMapNodeIdentifyPointOfChangeDefaultResult {
  mock: true;
  methodId: string;
...
export class MockWardleyMapNodeIdentifyPointOfChangeDefaultStrategy extends CoreBaseStrategy<WardleyMapNodeIdentifyPointOfChangeDefaultInput, WardleyMapNodeIdentifyPointOfChangeDefaultResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyMapNodeIdentifyPointOfChangeDefaultInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyMapNodeIdentifyPointOfChangeDefaultResult>> {
