// Mock strategy for `wardley:map:node:classify-point-of-change:default` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:map:node:classify-point-of-change:default';

export interface WardleyMapNodeClassifyPointOfChangeDefaultInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyMapNodeClassifyPointOfChangeDefaultResult {
  mock: true;
  methodId: string;
...
export class MockWardleyMapNodeClassifyPointOfChangeDefaultStrategy extends CoreBaseStrategy<WardleyMapNodeClassifyPointOfChangeDefaultInput, WardleyMapNodeClassifyPointOfChangeDefaultResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyMapNodeClassifyPointOfChangeDefaultInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyMapNodeClassifyPointOfChangeDefaultResult>> {
