// Mock strategy for `wardley:map:node:identify-method:project-management` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:map:node:identify-method:project-management';

export interface WardleyMapNodeIdentifyMethodProjectManagementInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyMapNodeIdentifyMethodProjectManagementResult {
  mock: true;
  methodId: string;
...
export class MockWardleyMapNodeIdentifyMethodProjectManagementStrategy extends CoreBaseStrategy<WardleyMapNodeIdentifyMethodProjectManagementInput, WardleyMapNodeIdentifyMethodProjectManagementResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyMapNodeIdentifyMethodProjectManagementInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyMapNodeIdentifyMethodProjectManagementResult>> {
