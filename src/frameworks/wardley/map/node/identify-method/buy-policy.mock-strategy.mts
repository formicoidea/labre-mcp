// Mock strategy for `wardley:map:node:identify-method:buy-policy` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:map:node:identify-method:buy-policy';

export interface WardleyMapNodeIdentifyMethodBuyPolicyInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyMapNodeIdentifyMethodBuyPolicyResult {
  mock: true;
  methodId: string;
...
export class MockWardleyMapNodeIdentifyMethodBuyPolicyStrategy extends CoreBaseStrategy<WardleyMapNodeIdentifyMethodBuyPolicyInput, WardleyMapNodeIdentifyMethodBuyPolicyResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyMapNodeIdentifyMethodBuyPolicyInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyMapNodeIdentifyMethodBuyPolicyResult>> {
