// Mock strategy for `wardley:map:node:generate-node-from-pipeline:default` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:map:node:generate-node-from-pipeline:default';

export interface WardleyMapNodeGenerateNodeFromPipelineDefaultInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyMapNodeGenerateNodeFromPipelineDefaultResult {
  mock: true;
  methodId: string;
...
export class MockWardleyMapNodeGenerateNodeFromPipelineDefaultStrategy extends CoreBaseStrategy<WardleyMapNodeGenerateNodeFromPipelineDefaultInput, WardleyMapNodeGenerateNodeFromPipelineDefaultResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyMapNodeGenerateNodeFromPipelineDefaultInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyMapNodeGenerateNodeFromPipelineDefaultResult>> {
