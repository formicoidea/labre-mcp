// Mock strategy for `wardley:map:value-chain:generate:default` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:map:value-chain:generate:default';

export interface WardleyMapValueChainGenerateDefaultInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyMapValueChainGenerateDefaultResult {
  mock: true;
  methodId: string;
...
export class MockWardleyMapValueChainGenerateDefaultStrategy extends CoreBaseStrategy<WardleyMapValueChainGenerateDefaultInput, WardleyMapValueChainGenerateDefaultResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyMapValueChainGenerateDefaultInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyMapValueChainGenerateDefaultResult>> {
