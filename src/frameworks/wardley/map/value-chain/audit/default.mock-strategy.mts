// Mock strategy for `wardley:map:value-chain:audit:default` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:map:value-chain:audit:default';

export interface WardleyMapValueChainAuditDefaultInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyMapValueChainAuditDefaultResult {
  mock: true;
  methodId: string;
...
export class MockWardleyMapValueChainAuditDefaultStrategy extends CoreBaseStrategy<WardleyMapValueChainAuditDefaultInput, WardleyMapValueChainAuditDefaultResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyMapValueChainAuditDefaultInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyMapValueChainAuditDefaultResult>> {
