// Mock strategy for `wardley:map:climate:inertia-identification:default` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:map:climate:inertia-identification:default';

export interface WardleyMapClimateInertiaIdentificationDefaultInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyMapClimateInertiaIdentificationDefaultResult {
  mock: true;
  methodId: string;
...
export class MockWardleyMapClimateInertiaIdentificationDefaultStrategy extends CoreBaseStrategy<WardleyMapClimateInertiaIdentificationDefaultInput, WardleyMapClimateInertiaIdentificationDefaultResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyMapClimateInertiaIdentificationDefaultInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyMapClimateInertiaIdentificationDefaultResult>> {
