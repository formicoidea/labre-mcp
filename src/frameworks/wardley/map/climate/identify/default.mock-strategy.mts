// Mock strategy for `wardley:map:climate:identify:default` (ast-schema.md v0.1.0 § 3.4 — status: "mock").
//
...
const METHOD_ID = 'wardley:map:climate:identify:default';

export interface WardleyMapClimateIdentifyDefaultInput {
  // Open shape — the real strategy will narrow this when implemented.
  // any: mock-strategy input is intentionally open
  [key: string]: unknown;
}

export interface WardleyMapClimateIdentifyDefaultResult {
  mock: true;
  methodId: string;
...
export class MockWardleyMapClimateIdentifyDefaultStrategy extends CoreBaseStrategy<WardleyMapClimateIdentifyDefaultInput, WardleyMapClimateIdentifyDefaultResult> {
  static get method(): string { return METHOD_ID; }

  async evaluate(
    _input: WardleyMapClimateIdentifyDefaultInput,
    _context: RequestContext,
  ): Promise<StrategyResult<WardleyMapClimateIdentifyDefaultResult>> {
