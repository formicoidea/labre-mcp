...
In Wardley Mapping, there is a critical distinction between:
- **SOLUTIONS**: Concrete products, platforms, services, frameworks, or technologies with a specific identity. They are created by specific companies or communities, have version numbers, brand names, or distinct identities. Examples: Kubernetes, Salesforce, SAP ERP, PostgreSQL, React, AWS Lambda, Terraform, Slack, Docker.
- **CAPABILITIES**: Abstract activities, practices, knowledge areas, or data types that describe WHAT needs to be done, not HOW. They transcend any specific implementation. Examples: container orchestration, customer relationship management, enterprise resource planning, relational data storage, front-end rendering, serverless compute, infrastructure-as-code, team communication, containerization.
...
Consider:
1. Does this name refer to a SPECIFIC product/platform/service/framework with a brand identity?
   → If yes, it is a SOLUTION.
2. Does this name describe a GENERAL activity, practice, knowledge area, or data type that could be fulfilled by multiple implementations?
   → If yes, it is a CAPABILITY.
3. Some names are ambiguous (e.g., "Git" is both a solution AND the only common implementation). In such cases, lean toward the most common usage in Wardley Mapping contexts.
