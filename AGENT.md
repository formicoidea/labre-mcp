...
> **labre-mcp is a DEAD END (ARCH-31, product arbitration 2026-08-26).** A harness — external or the labre app itself — enters for framework knowledge and competences, and comes back out with a result. **This repository never writes labre's business state** (conversations, messages, maps, turns): the `agentReply` liaison was built (ARCH-30), then rejected and reverted the same day, because it turned the dead end into a write gateway and had to vendor labre's conversation language to exist. What labre-mcp DOES write into labre's Postgres is its **accounting** — `ai_calls` rows, quota checks — because labre is this daemon's administration and billing plane; the tokens a client burns here are billed there, and the `lab_`/stdio metering gap is a defect to close, not a tolerance. **The `lab_` half is closed** (2026-08-26): a key-authenticated call now writes its `ai_calls` row and spends the key owner's labre quota, through the `record_mcp_key_spend` definer RPC — see ARCH-31's follow-through. The stdio half stays open BY DECISION: stdio carries no identity and spends the user's own provider keys, so `stdio local sans clé = non compté, assumé`. Any future conversation-write door is a **labre surface**, never this one.
...
- [decisions.md](/labre-mcp/docs/architecture/decisions.md) — 31 ADRs (ARCH-01..31) that ground every other decision. **ARCH-31** (accepted 2026-08-26) is the dead-end rule: accounting writes yes, business writes never — it rejects ARCH-30 (`agentReply`, reverted). **ARCH-27** (applied 2026-08-26) is the façade: three layers, the four cuts, and what stays at the delivery. **ARCH-28** is the costume: prompts + resources served from kernel catalogues, the prompt selection criterion, the `labre://` URI scheme, and the DATA-ONLY limit that reserves the plugin runtime for C4/CH-26. ARCH-26 settles who owns the `labre_mcp` Postgres schema: the migration chain stays in labre, this repo holds the mechanical schema contract. **ARCH-29 is ✅ accepted (2026-08-28): option (a), richer DATA-ONLY bundles, no executable plugin runtime** — the four open questions are answered there (first-party authors, HTTP only, no nameable methodId that data cannot express, signing overkill while the payload is inert). **The prohibition stands and is now permanent, not conditional on the old 🔴 status: no executable-plugin code, ever, under (a).** It lapses only if a later arbitration retains an executable option and says so in ARCH-29 first — and the day third parties may author plugins, ARCH-29 reopens whole rather than being amended
- [plugin-runtime-security.md](/labre-mcp/docs/architecture/plugin-runtime-security.md) — companion to ARCH-29: what DATA-ONLY protects (four named threats), the three options (rich data / minimal loader / Cordis), containment as an orthogonal axis, and the two guards as testable requirements. **The option analysis is settled — (a) retained 2026-08-28; the threat model and the guards are current state, and (b)/(c) remain the refused branches with the criterion that would revive them**
