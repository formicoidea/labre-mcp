...
- [decisions.md](/labre-mcp/docs/architecture/decisions.md) — the ADRs that ground every other decision (ARCH-01..29; ARCH-28 is reserved by a parallel chantier). **ARCH-27** (applied 2026-08-26) is the façade: three layers, the four cuts, and what stays at the delivery. ARCH-26 settles who owns the `labre_mcp` Postgres schema: the migration chain stays in labre, this repo holds the mechanical schema contract. **ARCH-29 is 🔴 proposed, not decided** — it re-opens the `bundle = DATA-ONLY` security model for the CH-26 plugin runtime and awaits human arbitration; no executable-plugin code may be written before it moves
